...
data modify storage proclamations:dialog dialog set value {\
  ...
  yes: {\
    action: {\
      ...
      template: "trigger proclamations.ui.player_detection_distance set $(detection_distance)"\
    },\
    label: "Confirm"\
  },\
  no: {\
...
data modify storage proclamations:dialog dialog.inputs[0].initial \
    set from storage proclamations:temp current_marker_data.triggers[0].distance
