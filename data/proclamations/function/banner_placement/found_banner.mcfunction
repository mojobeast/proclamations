...
tag @s remove proclamations.newly_summoned_marker

data modify entity @s data.proclamations merge from block ~ ~ ~ components."minecraft:custom_data".proclamations
data modify entity @s data.proclamations.triggers set value []
data modify entity @s data.proclamations.lore set from block ~ ~ ~ components."minecraft:lore"

execute if block ~ ~ ~ #minecraft:banners[facing=west] \
    align xyz positioned ~1 ~ ~ \
    run return \
    run function proclamations:banner_placement/found_attached_block
